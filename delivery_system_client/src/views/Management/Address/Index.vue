                        ...
                        <el-form :inline="true"
                            ...
                            <el-form-item label="联系电话">
                                <el-input placeholder="请输入联系电话"
                                            v-model="data.formList.phone"
                                            style="width: 200px"
                                            @keyup.enter.native="getData">
                                  </el-input>
                            </el-form-item>
                            <el-form-item label="对应用户ID">
                                <el-input placeholder="请输入对应用户ID"
                                            v-model="data.formList.accountId"
                                            style="width: 200px"
                                            @keyup.enter.native="getData">
                                  </el-input>
                            </el-form-item>
...
<script lang="ts" setup>
    ...
    const getData = () => {
        ...
        // 查询参数
        const params = {
            longitude : data.formList.longitude,
            latitude : data.formList.latitude,
            provinceName : data.formList.provinceName,
            provinceId : data.formList.provinceId,
            cityName : data.formList.cityName,
            cityId : data.formList.cityId,
            countyName : data.formList.countyName,
            countyId : data.formList.countyId,
            detailAddress : data.formList.detailAddress,
            contacts : data.formList.contacts,
            phone : data.formList.phone,
            accountId : data.formList.accountId,
            houseNumber : data.formList.houseNumber,
            pageIndex : data.pageConfig.currentPage,
            pageSize : data.pageConfig.pageSize
        }
        // 后台请求
        Api.selpage4address(params).then(res=> {
            if (res.code === 200){
    ...
    const uploadExcel = () => {
        ...
        uploadExcelRef.value.init(this.SHOP_SERVER + '/address/uploadExcel');
    }
    ...
    const deleteDataMany = () => {
        ...
        if (!datas || !datas.length || datas.length === 0) {
            ...
            ElMessageBox.confirm(
                ...
                Api.dels4address(dataids).then(res => {
                    if (res.code === 200){
                        ElMessage({
                            type: 'success',
                            message: '删除成功',
    ...
    const excelData = () => {
        const params = {}
        Api.excelData4address(params).then(data => {
            const blob = new Blob([data], { type: 'application/vnd.ms-excel' })
            const blobUrl = URL.createObjectURL(blob)
            const a = document.createElement('a')
            a.href = blobUrl
    ...
    const toDelete = (scope) => {
        ElMessageBox.confirm(
            ...
            Api.del4address(scope.row.id).then(res => {
                console.log(res)
                if (res.code === 200){
                    ElMessage({
                        type: 'success',
