          ...
          <div
            ...
            <el-form
              ...
              <el-form-item label="评分">
                <el-input
                        placeholder="请输入评分"
                      v-model="data.formList.mark"
                      style="width: 200px"
                      @keyup.enter.native="getData">
                </el-input>
              </el-form-item>
              <el-form-item label="商家回复ID">
                <el-input
                        placeholder="请输入商家回复ID"
                      v-model="data.formList.recoverId"
                      style="width: 200px"
                      @keyup.enter.native="getData">
                </el-input>
              </el-form-item>
...
<script lang="ts" setup>
    import Api from '../../api/api_shopcomment.js'
    ...
    const data = reactive({
        ...
        detailUrl: '/name/shopcomment/item', // detailUrl:详情页面地址
    ...
    const getData = () => {
        ...
        const params = {
            ...
            mark : data.formList.mark,
            recoverId : data.formList.recoverId,
            pageIndex : data.pageConfig.currentPage,
            pageSize : data.pageConfig.pageSize
        }
        // 后台请求
        Api.selpage4shopcomment(params).then(res=> {
            if (res.code === 200){
    ...
    const uploadExcel = () => {
        ...
        uploadExcelRef.value.init(this.SHOP_SERVER + '/shopcomment/uploadExcel');
    }
    ...
    const deleteDataMany = () => {
        ...
        if (!datas || !datas.length || datas.length === 0) {
            ...
            ElMessageBox.confirm(
                ...
                Api.dels4shopcomment(dataids).then(res => {
                    if (res.code === 200){
                        ElMessage({
                            type: 'success',
                            message: '删除成功',
    ...
    const excelData = () => {
        const params = {}
        Api.excelData4shopcomment(params).then(data => {
            const blob = new Blob([data], { type: 'application/vnd.ms-excel' })
            const blobUrl = URL.createObjectURL(blob)
            const a = document.createElement('a')
            a.href = blobUrl
    ...
    const toDelete = (scope) => {
        ElMessageBox.confirm(
            ...
            Api.del4shopcomment(scope.row.id).then(res => {
                console.log(res)
                if (res.code === 200){
                    ElMessage({
                        type: 'success',
